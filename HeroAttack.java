...
public class HeroAttack {
	...
	static void StartHeroAttack() throws InterruptedException {
			...
			if (selectMove == 0) {
				...
				for (int i = 0; i < heroStatus.solNum.size(); i++) {
					if (heroStatus.solNum.get(i).solHp > 0) {
						Thread.sleep(1000);
						System.out.println("\n" + heroStatus.solNum.get(i).solName + "의 공격입니다.");
	...
	static int heroAttack() {
		...
		while (true) {
			...
			} else if (heroStatus.selection == 2) {
			...
			} else if (heroStatus.selection == 3) {
			...
			} else if (heroStatus.selection == 4) {
	...
	static void heroAttacked(int sum) throws InterruptedException {
		Random r = new Random();
		if (sum < 0) {
			sum = 0;
		}
		while (true) {
			...
			if (target == heroStatus.solNum.size()) {
				Thread.sleep(1000);
				System.out.println("\n" + Monster.monsName + "이(가) " + heroStatus.name + "를 공격!");
				heroStatus.hp = heroStatus.hp - sum;
				Thread.sleep(1000);
				System.out.println("\n > " + heroStatus.name + "이(가) 받은 데미지는 " + sum + "입니다.");
				System.out.println("=====================================");
				break;
			} else if (heroStatus.solNum.get(target).solHp > 0) {
				Thread.sleep(1000);
				System.out.println("\n" + Monster.monsName + "이 " + heroStatus.solNum.get(target).solName + "를 공격!");
				heroStatus.solNum.get(target).solHp -= sum;
				Thread.sleep(1000);
				System.out.println("\n > " + heroStatus.solNum.get(target).solName + "이(가) 받은 데미지는 " + sum + "입니다.");
				System.out.println("=====================================");
